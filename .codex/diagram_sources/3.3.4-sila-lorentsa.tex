...
\definecolor{pathline}{HTML}{7C3AED}
...
\begin{tikzpicture}[
  ...
  \draw[pathline,line width=0.7mm,postaction={decorate},
    decoration={markings,mark=at position 0.55 with {\arrow{Latex}}}]
    (78,47) arc[start angle=245,end angle=5,radius=44mm];
  \node[font=\sffamily\fontsize{10}{12}\selectfont,text=pathline,align=center]
    at (164,34) {траектория\\положительного заряда};

  \draw[fill=chargefill,draw=chargeborder,line width=0.7mm] (94,70) circle (6.5);
  \node[font=\sffamily\bfseries\fontsize{13}{15}\selectfont,text=chargeborder]
    at (94,70) {$+$};
